...
export const SHARED_PROMPT =
  ...
  `All amounts are Indonesian Rupiah, where "." on the receipt is a THOUSANDS separator, not a decimal point (e.g. printed "50.000" means fifty thousand — output as 50000, never 50). Output every amount as the full integer Rupiah value with no separators. ` +
...
const GROQ_PROMPT =
  ...
  `Amounts are Indonesian Rupiah: "." on the receipt is a THOUSANDS separator, not decimal (printed "50.000" = fifty thousand → output 50000, never 50). Output full integer Rupiah, no separators. ` +
...
export async function callOpenRouter(apiKey, base64Image, mimeType, signal) {
  ...
  const response = await fetch(OPENROUTER_API_URL, {
    ...
    headers: {
      "Content-Type": "application/json",
      Authorization: `Bearer ${apiKey}`,
      "HTTP-Referer": "https://www.splitbill.my.id",
