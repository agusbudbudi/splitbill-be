...
function isLocalOrigin(origin) {
  ...
  // Local network IPs (192.168.x.x, 10.x.x.x, 172.16.x.x to 172.31.x.x)
  const privateIpRegex = /^(https?:\/\/)(192\.168\.|10\.|172\.(1[6-9]|2[0-9]|3[0-1])\.)/;
  return privateIpRegex.test(origin);
}
...
export function createCorsHeaders(event) {
  ...
  const isAllowedOrigin = isLocal || (origin && DEFAULT_ALLOWED_ORIGINS.includes(origin));
  ...
  const responseHeaders = {
    Vary: "Origin",
    // Only allow known origins, reject unknown ones
  ...
  return addSecurityHeaders(responseHeaders);
}
