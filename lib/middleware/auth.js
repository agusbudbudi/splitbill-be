...
export async function requireUser(event) {
  ...
  try {
    ...
    // ✅ SHOULD DO: Lazy subscription expiry check
    if (user.subscriptionStatus === "active" && 
        user.subscriptionExpiry && 
        user.subscriptionExpiry < new Date()) {
      user.subscriptionStatus = "expired";
      await user.save();
      console.log(`Subscription lazily expired for user: ${user.email}`);
    }
...
export async function requireAdmin(event) {
  ...
  if (!user.isAdmin) {
    ...
    logger.security("Non-admin user attempted to access admin endpoint", {
      userId: user._id,
      email: user.email,
    });
    throw new HttpError(
