...
const scanGuestLimiter = new RateLimiter(
  24 * 60 * 60 * 1000, // 24 hours
  1, // 1 scan per 24 hours per IP
);
...
/**
 * Apply rate limiting for AI scan API (by user ID)
 */
export function applyScanRateLimit(userId) {
  if (!userId) {
    throw new HttpError(401, "Authentication required");
  }
  const identifier = `scan:${userId}`;
  return scanLimiter.check(identifier);
}

/**
 * Apply rate limiting for AI scan API for guest/unauthenticated users (by IP)
 */
export function applyScanGuestRateLimit(event) {
  const ip = getClientIp(event);
  const identifier = `scan:guest:${ip}`;
  return scanGuestLimiter.check(identifier);
}
