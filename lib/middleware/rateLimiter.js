...
const geminiGuestLimiter = new RateLimiter(
  24 * 60 * 60 * 1000, // 24 hours
  2, // 2 scans per 24 hours per IP
);
...
/**
 * Apply rate limiting for Gemini API for guest/unauthenticated users (by IP)
 */
export function applyGeminiGuestRateLimit(event) {
  const ip = getClientIp(event);
  const identifier = `gemini:guest:${ip}`;
  return geminiGuestLimiter.check(identifier);
}
